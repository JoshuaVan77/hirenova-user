...
export default function WithdrawRecords() {
  ...
  // Backend ကနေ Withdraw History ကို ဆွဲယူခြင်း
  const fetchRecords = async () => {
    try {
      const response = await api.get('/user/withdraw-history');
    ...
    <div className="min-h-screen bg-dark-bg text-white pb-24 pt-4 px-3 sm:px-6 lg:px-8">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex items-center gap-4 mb-6">
          ...
          <h1 className="text-xl sm:text-2xl font-bold">Withdrawal History</h1>
        </div>
        ...
        {records.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20">
            ...
            <p className="text-gray-400 text-sm">No withdrawal records yet</p>
          </div>
        ) : (
